...
class DatabaseManager:
    ...
    def add_winner(self, user_id, prize_id):
        ...
        with conn:
            ...
            if cur.fetchall():
                ...
                conn.commit()
                time.sleep(5)  # Ожидание 5 секунд перед отправкой сообщения
                print(f'Приз отправлен пользователю {user_id}')
                return 1
    ...
    def get_users(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("SELECT user_id FROM users")  # Возвращаем user_id вместо user_name
